...
| **M8 The house answers** (hands-free, end to end) — **three sub-gates** | **M8a** node client + node audio + wake word on the device + echo cancellation + room attribution (FR-13 amended, FR-19); **M8b** automations (FR-17) + the daemon scheduler (closes D-M4-1); **M8c** settings surface, a real install, golden 12. | With **no browser open**: the wake word fires in the kitchen, the question is answered aloud *there*, a timer set by voice rings in the room it was set in, an automation fires on its own, every device is pairable/listable/revocable from the UI, and the whole thing starts from a documented install on a fresh machine. First real NFR-04 measurement (D-M5-3). |
| **M9 Product hardening** | Installer/update, backup/restore + restore test, policy UI, accessibility, diagnostics bundle, signed releases. (Was M8; the settings surface and the deployment story moved forward into M8c, because a hands-free house cannot be run without them.) | Repeatable install/upgrade/rollback; full security release checklist passes; golden 10. |
