...
| **M9 Load-bearing** (refactoring: maintenance and readability) ✅ (signed off 2026-09-01, gate approved; ADR-034 accepted; the `rfc3339` behaviour-consolidation deviation and unpinned-CI-tag risk accepted; tag `m9-complete`) | Untracked the 18 committed worktree gitlinks; CI caching; workspace dependency inheritance; a `jarvis-test-support` crate for the doubles written 7–8× over; split the god-modules (`spotify.rs` 3,789, `home_assistant.rs` 3,469, `ws.rs` 2,348, `runs.rs` 1,524, `config.rs` 1,311) and the 708-line composition root; one tool-declaration seam; one problem/time helper instead of `rfc3339` ×9 in two divergent variants; one web socket client and a shared card layer; **ADR-034** + intra-crate `arch-test` rules, `clippy.toml`/`rustfmt.toml`. No behaviour changes except the one accepted deviation below. | **Behaviour identical, provably:** 1,629 Rust tests / 315 web tests, 0 failed; every golden scenario (1–7, 9–12 + M3a/M3b/M5/M6 acceptance) passes unchanged; `cargo xtask codegen --check` passes *without regenerating*; the diff touches no `migrations/` file and leaves `.sqlx/` empty; idle RSS unchanged (23.0 MB vs. 22.9 MB pre-M9). **Structure measurably changed:** `cargo xtask arch-test` enforces no `.rs` file over **1,700 lines** and no function over **730 lines** — ADR-034 §3's ratchet to the tree's actual worst value at landing, not the 1,000/150 this row originally guessed at before F9.1–F9.12's real scope was known; test doubles come from one crate (`jarvis-test-support`); CI security job 121s → 31s. **All demonstrated** — see `docs/milestones/M9-gate-report.md`. |
