...
- CalDAV read-only agenda retrieval with HTTPS, same-origin enforcement, cancellation, byte/event bounds, sanitized/length-capped titles, and an audited read (the sensitivity label is not leaked to the client; every event is labeled `Sensitive` and titles are not otherwise suppressed — unlike memory retrieval, there is no relevance/suppression filtering on the agenda path).
- SMTP `message.send` as an opt-in R2 external tool with exact approval arguments, grant-required execution (now including a grant/invocation argument-fingerprint match), bounded transport, keyring-bound credentials, and send idempotency (a replayed approval no longer double-sends). The primary defense is the single-use grant consumed under `FOR UPDATE` in `PgGrantStore`; `InMemorySmtpIdempotencyStore` is a second, process-local belt that does not survive a restart — a durable store is documented future work.
