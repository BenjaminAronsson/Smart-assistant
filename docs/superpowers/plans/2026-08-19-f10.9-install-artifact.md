...
> **DELIVERED — PR #82, merged 2026-08-30.** All ten tasks are implemented; the checkboxes
> below were never ticked as the work went in, and are left as written rather than
> back-filled, because a tick added afterwards records nothing. What the work actually did,
> including the six defects the review found and the one CI found, is in
> `docs/milestones/M10-features.md` under F10.9. Two items in the Definition of Done remain
> and are owner-only: sign-off on F10.9's position in M10 and on the
> `/etc/jarvis/secrets.env` secret-at-rest trade, and the reboot test on real hardware.
