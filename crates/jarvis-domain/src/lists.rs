...
impl ItemList {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        out.push_str(if name.is_empty() { "untitled list" } else { &name });
        out.push('\n');
...
const FILLERS: &[&str] = &["jarvis", "hey", "ok", "okay", "please", "could", "can", "you"];
...
pub fn parse_list_command(utterance: &str) -> Option<ListCommand> {
    ...
    let read_tail: Option<&[Word]> = match head {
        ...
        "what" => (second == "is" && words.get(2).is_some_and(|w| w.norm == "on"))
            .then(|| &words[3..]),
...
mod tests {
    ...
    fn list_name_keys_fold_case_whitespace_and_a_trailing_list_word() {
        for spelling in ["Shopping", "shopping list", "  SHOPPING   LIST ", "Shopping"] {
            assert_eq!(
    ...
    fn duplicate_text_is_disambiguated_by_id_not_by_matching() {
        ...
        assert!(umlaut.find_by_text(&ItemText::new("MÜSLI").unwrap()).is_some());
    }
    ...
    fn promoted_markdown_keeps_our_structure_and_escapes_their_content() {
        ...
        assert_eq!(lines[0], "# \\# Shopping", "our heading, their text escaped");
